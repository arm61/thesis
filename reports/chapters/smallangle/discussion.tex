...
In order to assess the utility for the PSO, two \texttt{fitoog} runs were perfromed (each with five repetitions, and a population size of 96 over 500 iterations).
...
This inability for the PSO method to optimise the structure of the micellar species could be due to a wide varity of reasons, some which could be acted on and others that could not.
...
However, it may the case that the parameter space was too large to be optimisated using the PSO alone, this is very likely considering that the dimensionality of the parameter space was 4500 ($500\times6+500\times3$, for 500 \ce{C_{10}TA+} and \ce{NO3-} molecules).
...
A possible method that may enable the optimisation of such structures would the be inclusion of an energetic term.
...
Additionally, the Emperical Potential Structure Refinement used by Hargreaves \emph{et al.} \cite{hargreaves_atomistic_2011} and the method used by Ivanovic \emph{et al.} \cite{ivanovic_temperature-dependent_2018} both involve performing and energetic optimisation that is biased based on the agreement with experimental data. 

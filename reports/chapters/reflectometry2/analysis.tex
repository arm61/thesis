...
The second modification was to constrain the head group volume to a value of \SI{339.5}{\angstrom}, in agreement with the work of Ku\v{c}erka \emph{et al.}\sidecite{kucerka_determination_2004} and Balgav\'{y} \emph{et al.}\sidecite{balgavy_evaluation_2001}
...
As with the work of Chapter~\ref{reflectometry1}, Markov chain Monte Carlo sampling was used to obtain experimental uncertainties on the fitted model, enabled by the \texttt{emcee} package \sidecite{foreman-mackey_emcee_2013}.
...
A custom-class, \texttt{md\_simulation}, was developed for \texttt{refnx}\sidecite{nelson_refnx_2019,nelson_refnx_2019-1} that enabled the determination of a reflectometry profile from simulation, using a similar method to that employed in previous work, such as Dabkowska \emph{et al.}\sidecite{dabkowska_modulation_2014}
...
The solvent penetration was determined using the intrinsic surface approach, as detailed by Allen \emph{et al.}\sidecite{allen_specific_2016,pandit_algorithm_2003}
...
All of these analyses used the \texttt{MDAnalysis} package.\sidecite{gowers_mdanalysis_2016,michaud-agrawal_mdanalysis_2011}

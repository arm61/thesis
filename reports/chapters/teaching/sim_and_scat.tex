...
Secondly, as the source-code repository used to build these webpages.\sidecite{mccluskey_pythoninchemistry/sim_and_scat_2019}
The source content consists of a set of Jupyter Notebooks and Markdown files, which are automatically compiled using the \texttt{jupyter-book} tool\sidecite{lau_jupyter/jupyter-book_2019} to generate the web version.
...
The web pages have Thebelab and BinderHub integrations,\sidecite{ragan-kelley_minrk/thebelab_2019, ragan-kelley_jupyterhub/binderhub_2019, jupyter_binder_2018} which allow students to launch interactive versions of these webpages that allow execution and modification of the included Python code.
The ability to read the resource as an ``interactive document'' improves the ability for the students to engage in the ``worked example effect''.\sidecite{tarmizi_guidance_1988}
The resource is provided under a CC-BY license,\sidecite{creative_commons_creative_2019} while the \texttt{jupyter-book} software is shared under an MIT license,\sidecite{open_source_mit_2019} both of which are open and highly permissive.
...
Including the use of interatomic potential functions, alongside some examples, such as the Lennard-Jones and Buckingham potential models.\sidecite{lennard-jones_determination_1924,buckingham_classical_1938}
...
It is shown how a one dimensional NVE (constant number, volume, and energy) MD simulation may be built, using the Velocity-Verlet algorithm and the Lennard-Jones potential model.\sidecite{swope_computer_1982,lennard-jones_determination_1924}
...
This is presented as a practical example, using the \texttt{pylj} package.\sidecite{mccluskey_pylj_2018,mccluskey_arm61/pylj_2019-2}
...
Next, the Debye equation\sidecite{debye_zerstreuung_1915} is presented and it is shown how it may be used to calculate scattering data from a simulation.
The user is invited to observe the effect of simulation temperature on the resulting scattering profile.
There is also a short discussion of alternative, faster, algorithms for calculating scattering profiles, such as the Fibonacci Sequence or Golden Vectors method.\sidecite{watson_rapid_2013,svergun_solution_1994}

\textbf{``Real'' simulation and scattering:} Having shown the development of a scattering profile from an idealised system, the user is directed to a popular resource for the GROMACS\sidecite{berendsen_gromacs_1995} MD software.
This resource gives a quick introduction to using GROMACS to simulate a lysozyme molecule in buffer.\sidecite{lemkul_gromacs_nodate}
The user may then use their own simulated trajectory or one that can be downloaded from the OER.
It is shown how the system may be visualised, introducing the \texttt{MDAnalysis} Python package for the analysis of MD trajectories,\sidecite{michaud-agrawal_mdanalysis_2011,gowers_mdanalysis_2016} and show how the scattering profile developed from the lysozyme simulation compared with experimental data.\sidecite{franke_correlation_2015}
The module finishes by pointing the student to resources to more easily resolve scattering data from the molecular simulation, such as SASSIE and CRYSOL.\sidecite{perkins_atomistic_2016,svergun_crysol_1995}

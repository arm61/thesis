...
\texttt{pylj} (PYthon Lennard-Jones) \cite{mccluskey_pylj_2018,mccluskey_arm61/pylj_2019} is an educational software and molecular dynamics engine designed to introduce students to the details of classical simulation.
...
In an effort to reduce the complexity and visual subtely of typical molecular dynamics simulations, pylj performs two-dimension molecular dynamics or Monte-Carlo simulation.
...
\texttt{pylj} was designed such that it may be operated at a series of different levels of astraction.
...
The computational intensive nature of the pairwise force and energy calculations necessary for molecular dynamics and Monte Carlo simulation necessitated the use of Cython, a method of including compiled C language code within a Python pacakge \cite{noauthor_cython_nodate}.
This enabled a \num{\sim10} times speed up in the determination of the pairwise interactions when compared with the use of a pure Python implementation.
...
Furthermore, this speed is now comparable to the length of time taken to render the visualisation, meaning that the pairwise interactions is no longer the rate determing step in the use of \texttt{pylj}.
...
Plots such as instanteous pressure against time, radial distribution function, and instanteous temperature histogram are included in \texttt{pylj} and it is easy for the users to create custom plots or build visualisations where multiple plots can be presented together.
...
\texttt{pylj} was recently used by Dr Benjamin Morgan for a demonstration such as this in a seminar introducing chemical simulation to a cohort of mathematicians.
%
\begin{figure}
    \centering

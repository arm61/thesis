...
There is growing interest in the use of multi-modal analysis methods that leverage classical simulation to assist in the analysis of scattering data.\sidecite{ivanovic_temperature-dependent_2018,scoppola_combining_2018,dabkowska_modulation_2014,hub_interpreting_2018}
...
The potential energies of the short-range interactions are usually modelled as a combination of the attractive London dispersion interaction and the repulsive exchange forces that arise from the Pauli exclusions principle.\sidecite{leach_molecular_1996}
These are often forms such as shown below for the Lennard-Jones potential model,\sidecite{lennard-jones_determination_1924}
%
\begin{equation}
\begin{aligned}
...
The Lennard-Jones potential model is not the only form that may be used for the modelling of the short-range non-bonded interactions, others such as the Buckingham and Morse potentials exist.\sidecite{buckingham_classical_1938, morse_diatomic_1929}
...
While the short-range interactions can be accounted for by a function such as the Lennard-Jones potential model, the potential energy of the long-range electrostatic interactions are usually modelled, more consistently, using Coulomb's law for classical electrostatic interaction between point particles,\sidecite{coulomb_premier_1788, coulomb_second_1788}
%
\begin{equation}
...
The fact that Equation~\ref{equ:col} contains a factor of $r^2$ indicates that this is a much longer range interaction than those modelled with the Lennard-Jones model, make the Coulomb potential more complex to compute.\sidecite{frenkel_understanding_1996}

An example of a very large classical simulation would be $\sim3$ million atoms.\sidecite{gumbart_regulation_2009}
...
In order to avoid truncation artefacts, the Ewald summation is often used for the calculation of the electrostatic contribution to the potential energy.\sidecite{ewald_berechnung_1921}
...
Most modern MD simulation software packages implement the Ewald summation using a particle mesh Ewald method.\sidecite{essmann_smooth_1995}
...
An example includes the OPLS potential model which was parameterised by comparison to quantum mechanical measurements and crystallographic data.\sidecite{jorgensen_opls_1988}
...
This is the definition of super-atoms, in the place of groups of atoms, known as ``beading'', some examples are shown for the MARTINI force field\sidecite{marrink_martini_2007} in Figure~\ref{fig:cg}.
...
In addition to the computational benefit of having fewer particles in the simulation,\footnote{Therefore requiring fewer integrations of the equations of motion} there is also the opportunity to increase the timestep length for the simulation.\sidecite{pluhackova_biomembranes_2015}

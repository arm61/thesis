...
This is in contrast to inelastic scattering where energy transfer occurs; facilitating the measurement of system dynamics, such as the dynamical modes of polymers and phospholipid bilayers\sidecite{garcia_sakai_quasielastic_2009, farago_recent_2009}
%
\begin{figure}[t]
    \forceversofloat
    \centering
...
This is instrument-specific, although it may be approximated by convolving the experimental dataset with some Gaussian smearing function, the modelled intensity can then be determined from,\sidecite{nelson_towards_2013-1,nelson_towards_2014}
%
\begin{equation}
...
Transmission SAS can provide information about the size, shape and orientation of the sample's components.\sidecite{willis_experimental_2009}
...
This allows the SANS2D instrument, at the ISIS Neutron and Muon Source, to have a total range from \SIrange{2e-3}{2}{\per\angstrom}.\sidecite{noauthor_isis_nodate}
...
It is often possible to control the presence of the structure factor by changing the concentration of the sample, eventually, the concentration will be so low that all interparticle interaction is screened by the solvent.\sidecite{edler_combining_2015}
...
This scattering law is only valid at very small values of $q$, where $q < R_g^{-1}$,\sidecite{sivia_elementary_2011}
%
\begin{equation}
...
\end{equation}
%
The Guinier analysis is very common in the study of proteins by SAS, as it allows for the determination of the protein size in the native, solution phase.\sidecite{skou_synchrotron-based_2014}
...
This means that by plotting $I(q)q^4$ vs. $q$ and extrapolating to $q \rightarrow \infty$, it is possible to determine the external surface area of the system.\sidecite{willis_experimental_2009}
...
\end{equation}
%
where $N_\rho$ is the number density of the particles, $\Delta\text{SLD}$ is the difference in scattering length density between the particles and the solvent, $V_p$ is the particle volume, $P(q)$ is the particle form factor, and $S(q)$ is the structure factor.\sidecite{pedersen_monte_2002}
...
The solution for a sphere was solved in the early 19th century by Lord Rayleigh.\sidecite{pedersen_monte_2002}
%
\begin{equation}
...
Analytical form factors exist for a wide variety of shapes; these can be found in software such as SASView and SASFit.\sidecite{noauthor_sasview_nodate, noauthor_sasfit_nodate}
%
\begin{figure}[t]
    \centering
...
Structure factor expressions are generated as solutions to the Ornstein-Zernike Equation.\sidecite{klein_interacting_2002}
The most relevant structure factor in terms of micelle modelling is probably the Hayter-Penfold Mean Spherical Approximation, \sidecite{hayter_analytic_1981} this is where the micelles are modelled as like-charged, soft spheres and is valid for dilute solutions.
Again, a whole range of these structure factor functions are built into the SASView package.\sidecite{noauthor_sasview_nodate}

In order to evaluate the scattering profile from an atomic structure,\footnote{Or any coordinate system, such as a coarse-grained simulation.} it is possible to use the Debye equation.\sidecite{debye_zerstreuung_1915}
...
In order to improve the efficiency of the calculation of the scattering profile, a variety of methods have been developed that offer a sufficiently accurate approximation.\sidecite{svergun_solution_1994,watson_rapid_2013}
The Golden Vector method, developed by Watson and Curtis,\sidecite{watson_rapid_2013} scales as $\mathcal{O}(Nn)$, where $n$ is the number of scattering vectors used in the calculation.
...
This was first developed such that $n$ is a number from the Fibonacci sequence,\sidecite{svergun_solution_1994} however for the Golden Vector method, $n$ may be any positive integer.
...
\end{equation}
%
The accuracy of this calculation increases with $n$, however agreement, comparible to the Debye equation, between experiment and simulation has been shown for $n < 100$ even for highly anisotropic systems.\sidecite{watson_rapid_2013}

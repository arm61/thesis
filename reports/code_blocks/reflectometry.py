...
def abeles(q_values, sld, d):
    """
    Calculates the reflectometry from a set of layers of a given scattering
    length density.

    Parameters
    ----------
    q_values: float, array-like
        The q-vector values over which the reflectometry is to be calculated.
    sld: float, array-like
        An array of scattering length densities of length N, where N is the
        number of layers present.
    d: float, array-like
        An array of thicknesses of length N, where N is the number of layers
        present.
    ...
    kn = np.sqrt(q_values[:, np.newaxis] ** 2.0 / 4.0 - 4.0 * np.pi * sld)

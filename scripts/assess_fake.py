...
data3 = pd.read_csv('../data/smallangle/best_fake/best{}.xyz'.format(num), delim_whitespace=True, header=None, skiprows=2)

intensity2 = np.zeros_like(qs)
for e, q in enumerate(qs):
    for m in range(0, data3[1].size-1):
        for n in range(m+1, data3[1].size):
            bi = data.values[data[1] == data3[0][m]][0][5]
            bj = data.values[data[1] == data3[0][n]][0][5]
            xdist = data3[1][n] - data3[1][m]
            ydist = data3[2][n] - data3[2][m]
            zdist = data3[3][n] - data3[3][m]
            r_mn = np.sqrt(np.square(xdist) + np.square(ydist) + np.square(zdist))
            intensity2[e] += bi * bj * np.sin(
                r_mn * q) / (r_mn * q)
    if intensity2[e] < 0:
        intensity2[e] = 0
